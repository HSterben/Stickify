...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    if (deferInit && typeof requestIdleCallback !== "undefined") {
      idleId = requestIdleCallback(init, { timeout: 1200 });
    } else if (deferInit) {
      idleId = window.setTimeout(init, 32);
    } else {
      init();
    }
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const { program } = ctx;
    const u = program.uniforms as Record<string, { value: UniformValue }>;
